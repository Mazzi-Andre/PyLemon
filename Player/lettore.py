...
class MusicPlayer:
   ...
   #     self.lst.insert(1, "Python")
    #    self.lst.insert(2, "Perl")
        ...
        self.pb.pack() #riordinare sequenza oggetti
    ...
    def loadfolder(self):
        ...
        self.dirs = os.listdir(sourcePath) #ritorna lista contente nomi del path dato
        self.lst_pos = self.lst.size() # in modo che parto da ultimo
        for file in self.dirs:
            if file:
                if file.endswith('.mp3'):
                    self.lst.insert(self.lst_pos,sourcePath + '/' + file) #a lui gli compare quello perchè lo ha messo!!!
